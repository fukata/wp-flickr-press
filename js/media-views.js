    ...
    initialize: function(ev){
      ...
      fp['util'] = {
        ...
        generateHtmlTitle: function(photo, input) {
          if (input["embed_slideshow"] == "1" && "photosets" == that.props.get('wpfp_type') && that.props.get('wpfp_photoset') && that.props.get("result")["photoset"]) {
            return that.props.get("result")["photoset"]["title"];
          } else {
            return photo.title;
          }
        },
        generateHtmlDescription: function(photo, input) {
          if (input["embed_slideshow"] == "1" && "photosets" == that.props.get('wpfp_type') && that.props.get('wpfp_photoset')) {
            return "";
          } else {
